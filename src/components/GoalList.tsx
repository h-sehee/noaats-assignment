...
export default function GoalList() {
  ...
  const [showOnlyMainBankByGoal, setShowOnlyMainBankByGoal] = useState<Record<string, boolean>>({});
  ...
  const fetchRecommendations = async (goalId: string, term: number) => {
    ...
    try {
      ...
      const aiReadyData = filterAndSortSimple(
        ...
        !!showOnlyMainBankByGoal[goalId],
      );
      console.log("AI Ready Data:", aiReadyData);

      if (aiReadyData.length === 0) {
        const onlyMain = !!showOnlyMainBankByGoal[goalId];
        alert(
          onlyMain
              ...
              <>
                ...
                {goal.isLoading && (
                  <div className="py-12 text-center text-gray-400 text-sm animate-pulse">
                    {user?.displayName}님의 정보를 바탕으로 최적의 상품을
                    계산하고 있습니다...
                  </div>
                )}
              </>
